...
export default function AnnouncementDetailPage({ params }) {
  ...
  const fetchAnnouncementDetail = async () => {
    try {
      setLoading(true);
      const response = await postDetailsAPI.getPostDetail(announcementId, true);

      if (response.success) {
        setAnnouncementData(response.data);
      } else {
        setError(response.message || "ไม่พบข้อมูลประกาศ");
      }
    } catch (error) {
  ...
  const getTotalMediaCount = () => {
    const photos = announcementData?.photos?.length || 0;
    const videos = announcementData?.videos?.length || 0;
    const pdfs = announcementData?.pdfs?.length || 0;
        ...
        <div className="bg-white bg-opacity-95 rounded-2xl shadow-lg backdrop-blur-sm overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="bg-white rounded-full w-16 h-16 flex items-center justify-center">
                  <span className="text-[#01385f] font-bold text-xl">
                    {getContentTypeIcon(announcementData)}
                  </span>
                </div>
                <div>
                  <h1 className="text-white text-xl md:text-2xl font-bold drop-shadow">
                    {announcementData?.title_name}
                  </h1>
                  <p className="text-white opacity-90">
                    ประเภท: {announcementData?.type_name || "ป้ายประกาศ"}
                  </p>
                </div>
              </div>
              <div className="flex flex-col gap-2 text-right">
                <div className="text-white text-sm opacity-90">
                  วันที่ประกาศ: {formatDate(announcementData?.date)}
                </div>
                <div className="text-white text-sm opacity-90">
                  สร้างเมื่อ: {formatDate(announcementData?.created_at)}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-lg">
              ...
              {announcementData?.topic_name && (
                <h3 className="text-lg text-gray-700 mb-3">
                  {announcementData.topic_name}
                </h3>
              )}
              <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-3">
                <span>📸 {announcementData?.photos?.length || 0} รูปภาพ</span>
                <span>🎥 {announcementData?.videos?.length || 0} วิดีโอ</span>
                <span>📄 {announcementData?.pdfs?.length || 0} เอกสาร</span>
                <span>📎 {getTotalMediaCount()} ไฟล์ทั้งหมด</span>
              </div>
              <div className="inline-block px-3 py-1 rounded-full text-white text-sm font-medium bg-[#01bdcc]">
                {announcementData?.type_name || "ป้ายประกาศ"}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6 border-b border-gray-200">
              ...
              {announcementData?.photos && announcementData.photos.length > 0 && (
                <button
                ...
                >
                  📸 รูปภาพ ({announcementData.photos.length})
                </button>
              )}
              {announcementData?.videos && announcementData.videos.length > 0 && (
                <button
                ...
                >
                  🎥 วิดีโอ ({announcementData.videos.length})
                </button>
              )}
              {announcementData?.pdfs && announcementData.pdfs.length > 0 && (
                <button
                ...
                >
                  📄 เอกสาร ({announcementData.pdfs.length})
                </button>
              )}
            </div>
            ...
            <div className="min-h-[400px]">
                ...
                <div className="space-y-6">
                  {announcementData?.details ? (
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-gray-800 mb-4">รายละเอียดประกาศ</h3>
                      <div
                        className="prose max-w-none text-gray-700 leading-relaxed"
                        dangerouslySetInnerHTML={{ __html: announcementData.details }}
                      />
                    </div>
                  ) : (
                    <div className="text-center py-12 text-gray-500">
                      <span className="text-6xl">📄</span>
                      <p className="text-xl mt-4">ไม่มีรายละเอียดในประกาศนี้</p>
                    </div>
                  )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {announcementData?.photos && announcementData.photos.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{announcementData.photos.length} ไฟล์</p>
                          </div>
                        )}
                        {announcementData?.videos && announcementData.videos.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{announcementData.videos.length} ไฟล์</p>
                          </div>
                        )}
                        {announcementData?.pdfs && announcementData.pdfs.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{announcementData.pdfs.length} ไฟล์</p>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div>
                  {announcementData?.photos && announcementData.photos.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {announcementData.photos.map((photo, index) => (
                        <div key={photo.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                          <img
                            ...
                            onClick={() => window.open(createFileUrl(photo.post_photo_file), '_blank')}
                          />
                          <div className="p-3">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  onClick={() => handleDownload(photo.post_photo_file, `รูปภาพ-${index + 1}.jpg`)}
                    ...
                    <div className="text-center py-12 text-gray-500">
                      <span className="text-6xl">📸</span>
                      <p className="text-xl mt-4">ไม่มีรูปภาพในประกาศนี้</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div>
                  {announcementData?.videos && announcementData.videos.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {announcementData.videos.map((video, index) => (
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-gray-500">
                                วันที่อัพโหลด: {formatDate(video.created_at)}
                              </div>
                            </div>
                            <div className="flex flex-col gap-2">
                              <button
                                onClick={() => window.open(createFileUrl(video.post_video_file), '_blank')}
                                className="px-3 py-1 bg-green-500 text-white rounded text-xs hover:bg-green-600 transition-colors whitespace-nowrap"
                              >
                                เล่น
                              </button>
                              <button
                                onClick={() => handleDownload(video.post_video_file, `วิดีโอ-${index + 1}`)}
                    ...
                    <div className="text-center py-12 text-gray-500">
                      <span className="text-6xl">🎥</span>
                      <p className="text-xl mt-4">ไม่มีวิดีโอในประกาศนี้</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {activeTab === "pdfs" && (
                <div>
                  {announcementData?.pdfs && announcementData.pdfs.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {announcementData.pdfs.map((pdf, index) => {
                          ...
                          <div key={pdf.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-gray-500">
                                  วันที่อัพโหลด: {formatDate(pdf.created_at)}
                                </div>
                              </div>
                              <div className="flex flex-col gap-2">
                                <button
                    ...
                    <div className="text-center py-12 text-gray-500">
                      <span className="text-6xl">📄</span>
                      <p className="text-xl mt-4">ไม่มีเอกสารในประกาศนี้</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="text-sm text-gray-500">
                อัพเดทล่าสุด: {formatDate(announcementData?.updated_at)}
              </div>
              <div className="flex gap-3">
                ...
                <Link href="/announcement">
                  <button className="px-4 py-2 bg-[#01bdcc] text-white rounded-lg hover:bg-[#01a5b3] transition-colors">
                    กลับหน้าประกาศ
                  </button>
                </Link>
              </div>
