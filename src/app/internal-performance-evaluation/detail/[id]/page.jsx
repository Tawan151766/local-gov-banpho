...
export default function InternalPerformanceEvaluationDetailPage({ params }) {
  ...
  const fetchDetail = async () => {
    try {
      setLoading(true);
      const response = await postDetailsAPI.getPostDetail(postId, true);
      if (response.success) {
        ...
        <div className="bg-white bg-opacity-95 rounded-2xl shadow-lg backdrop-blur-sm overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="bg-white rounded-full w-16 h-16 flex items-center justify-center">
                  <span className="text-[#01385f] font-bold text-xl">{getContentTypeIcon(data)}</span>
                </div>
                <div>
                  <h1 className="text-white text-xl md:text-2xl font-bold drop-shadow">{data?.title_name}</h1>
                  <p className="text-white opacity-90">ประเภท: {data?.type_name || "การประเมินประสิทธิภาพภายใน"}</p>
                </div>
              </div>
              <div className="flex flex-col gap-2 text-right">
                <div className="text-white text-sm opacity-90">วันที่ประกาศ: {formatDate(data?.date)}</div>
                <div className="text-white text-sm opacity-90">สร้างเมื่อ: {formatDate(data?.created_at)}</div>
              </div>
            </div>
          </div>
          {/* Content Body */}
          <div className="p-6 md:p-8">
            ...
            <div className="mb-6 p-4 bg-blue-50 rounded-lg">
              <h2 className="text-2xl font-bold text-[#01385f] mb-2">{data?.title_name}</h2>
              {data?.topic_name && <h3 className="text-lg text-gray-700 mb-3">{data.topic_name}</h3>}
              <div className="flex flex-wrap gap-4 text-sm text-gray-600 mb-3">
                <span>📸 {data?.photos?.length || 0} รูปภาพ</span>
                <span>🎥 {data?.videos?.length || 0} วิดีโอ</span>
                <span>📄 {data?.pdfs?.length || 0} เอกสาร</span>
                <span>📎 {getTotalMediaCount()} ไฟล์ทั้งหมด</span>
              </div>
              <div className="inline-block px-3 py-1 rounded-full text-white text-sm font-medium bg-[#01bdcc]">{data?.type_name || "การประเมินประสิทธิภาพภายใน"}</div>
            </div>
            {/* Tab Navigation */}
            <div className="flex flex-wrap gap-2 mb-6 border-b border-gray-200">
              ...
              {data?.photos && data.photos.length > 0 && (
                <button onClick={() => setActiveTab("photos")} className={`px-4 py-2 rounded-t-lg font-medium transition-colors ${activeTab === "photos" ? "bg-[#01bdcc] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>📸 รูปภาพ ({data.photos.length})</button>
              )}
              {data?.videos && data.videos.length > 0 && (
                <button onClick={() => setActiveTab("videos")} className={`px-4 py-2 rounded-t-lg font-medium transition-colors ${activeTab === "videos" ? "bg-[#01bdcc] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>🎥 วิดีโอ ({data.videos.length})</button>
              )}
              {data?.pdfs && data.pdfs.length > 0 && (
                <button onClick={() => setActiveTab("pdfs")} className={`px-4 py-2 rounded-t-lg font-medium transition-colors ${activeTab === "pdfs" ? "bg-[#01bdcc] text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>📄 เอกสาร ({data.pdfs.length})</button>
              )}
            </div>
            {/* Tab Content */}
                ...
                <div className="space-y-6">
                  {data?.details ? (
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                      ...
                      <div className="prose max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: data.details }} />
                    </div>
                  ) : (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {data?.photos && data.photos.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{data.photos.length} ไฟล์</p>
                          </div>
                        )}
                        {data?.videos && data.videos.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{data.videos.length} ไฟล์</p>
                          </div>
                        )}
                        {data?.pdfs && data.pdfs.length > 0 && (
                          <div className="bg-white rounded-lg p-4 text-center">
                            ...
                            <p className="text-xs text-gray-500">{data.pdfs.length} ไฟล์</p>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              <div className="mt-8 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                  <div className="bg-white p-3 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">{data?.photos?.length || 0}</div>
                    <div className="text-sm text-gray-600">รูปภาพ</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg">
                    <div className="text-2xl font-bold text-red-600">{data?.videos?.length || 0}</div>
                    <div className="text-sm text-gray-600">วิดีโอ</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg">
                    <div className="text-2xl font-bold text-orange-600">{data?.pdfs?.length || 0}</div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="text-sm text-gray-500">อัพเดทล่าสุด: {formatDate(data?.updated_at)}</div>
              <div className="flex gap-3">
                ...
                <Link href="/internal-performance-evaluation"><button className="px-4 py-2 bg-[#01bdcc] text-white rounded-lg hover:bg-[#01a5b3] transition-colors">กลับหน้ารายการ</button></Link>
              </div>
            </div>
          </div>
        </div>
