...
export default function GeneralRequestsManagement() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const response = await fetch(`/api/general-requests?${params}`);
  ...
  const handleUpdateStatus = async (values) => {
    try {
      const response = await fetch(`/api/general-requests/${selectedRequest.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`/api/general-requests/${id}`, {
        method: 'DELETE',
      });
    ...
    <div className="p-6">
      <Card>
        <div className="mb-4 flex justify-between items-center">
          <h2 className="text-xl font-bold">จัดการคำร้องทั่วไป</h2>
        </div>

        <div className="mb-4 flex gap-4">
          <Input.Search
            placeholder="ค้นหาชื่อผู้ยื่นหรือเรื่อง..."
            allowClear
        ...
        {selectedRequest && (
          <div>
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="สัญชาติ" span={1}>
              ...
              <Descriptions.Item label="ที่อยู่" span={2}>
                ...
                {` ตำบล${selectedRequest.requester_subdistrict}`}
                {` อำเภอ${selectedRequest.requester_district}`}
                {` จังหวัด${selectedRequest.requester_province}`}
                {selectedRequest.requester_postal_code && ` ${selectedRequest.requester_postal_code}`}
              </Descriptions.Item>
              <Descriptions.Item label="เบอร์โทร" span={1}>
                {selectedRequest.requester_phone}
              </Descriptions.Item>
              <Descriptions.Item label="เรื่อง" span={2}>
                {selectedRequest.request_subject}
              </Descriptions.Item>
            ...
            {(selectedRequest.document_1 || selectedRequest.document_2 || selectedRequest.document_3 || 
              selectedRequest.other_document_1 || selectedRequest.other_document_2) && (
              <div className="mt-4">
                ...
                <div className="space-y-2">
                  ...
                  {selectedRequest.other_document_1 && (
                    <div className="flex items-center gap-2">
                      <DownloadOutlined />
                      <a
                        href={`https://banpho.sosmartsolution.com/storage/${selectedRequest.other_document_1}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 hover:underline"
                      >
                  ...
                  {selectedRequest.other_document_2 && (
                    <div className="flex items-center gap-2">
                      <DownloadOutlined />
                      <a
                        href={`https://banpho.sosmartsolution.com/storage/${selectedRequest.other_document_2}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-blue-600 hover:underline"
                      >
