...
export async function POST(request) {
  ...
  try {
    ...
    if (!subTopicId) {
      return NextResponse.json(
        { success: false, error: 'Sub Topic ID is required' },
        { status: 400 }
      );
    }

    // Connect to database and verify sub topic exists
    connection = await mysql.createConnection(dbConfig);
    
    const [subTopicExists] = await connection.execute(
      `SELECT st.id, st.topic_name, s.section_name, t.type_name
    ...
    if (subTopicExists.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Invalid sub topic ID' },
        { status: 400 }
      );
    }
    ...
    if (hasNewColumns) {
      ...
      [result] = await connection.execute(
        `INSERT INTO perf_results_files 
         (sub_topic_id, files_path, files_type, original_name, file_size, description) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
          subTopicId, 
          filePath,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        size: file.size,
        file_type: fileType,
        topic_name: subTopicExists[0].topic_name,
        section_name: subTopicExists[0].section_name,
        type_name: subTopicExists[0].type_name,
