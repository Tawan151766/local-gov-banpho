...
export async function POST(request) {
  ...
  try {
    ...
    // Connect to database and verify section exists
    connection = await mysql.createConnection(dbConfig);
    
    const [sectionExists] = await connection.execute(
      'SELECT id, section_name FROM laws_regs_sections WHERE id = ?',
      [sectionId]
    );
    ...
    if (hasNewColumns) {
      ...
      [result] = await connection.execute(
        `INSERT INTO laws_regs_files 
         (section_id, files_path, files_type, original_name, file_size, description) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [
          sectionId, 
          filePath,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        size: file.size,
        file_type: fileType,
        section_name: sectionExists[0].section_name,
  ...
  try {
    ...
    const [fileRecord] = await connection.execute(
      `SELECT 
         ...
         s.section_name,
         t.type_name
       FROM laws_regs_files f
       LEFT JOIN laws_regs_sections s ON f.section_id = s.id
       LEFT JOIN laws_regs_types t ON s.type_id = t.id
       WHERE f.id = ?`,
      [fileId]
    );
